...
class PythonDistribution:
    ...
    def get_paths(self):
        ...
        if manifest_full_path:
            ...
            sp_dir = f"{get_python_site_packages_short_path(python_version)}/"
            prepend_metadata_dirname = basename(manifest_full_path) == "installed-files.txt"
            if prepend_metadata_dirname:
                path_prepender = f"{basename(dirname(manifest_full_path))}/"
            else:
                path_prepender = ""
    ...
    def get_conda_dependencies(self):
        ...
        """
        python_spec = f"""python {".".join(self.python_version.split('.')[:2])}.*"""
...
def _is_literal(o):
    return False if not isinstance(o, str) or not o else o[0] in '\'"'
