...
def make_install_plan(conda_prefix):
    ...
    if site_packages_dir:
        ...
        print(
            f"WARNING: Cannot install xonsh wrapper without a python interpreter in prefix: {conda_prefix}",
            file=sys.stderr,
        )
...
def make_entry_point(target_path, conda_prefix, module, func):
    ...
    if isfile(conda_ep_path):
        original_ep_content = Path(conda_ep_path).read_text()
    else:
        original_ep_content = ""
...
def install_anaconda_prompt(target_path, conda_prefix, reverse):
    ...
    args = (
        "/K",
        f'""{join(conda_prefix, "condabin", "conda_hook.bat")}" && "{join(conda_prefix, "condabin", "conda_auto_activate.bat")}""',
    )
...
def _install_file(target_path, file_content):
    original_content = Path(target_path).read_text() if isfile(target_path) else ""
...
def install_Scripts_activate_bat(target_path, conda_prefix):
    ...
    file_content = Path(src_path).read_text()
    return _install_file(target_path, file_content)
...
def install_condabin_conda_bat(target_path, conda_prefix):
    ...
    file_content = Path(conda_bat_src_path).read_text()
    return _install_file(target_path, file_content)
...
def install_conda_psm1(target_path, conda_prefix):
    ...
    file_content = Path(conda_psm1_path).read_text()
    return _install_file(target_path, file_content)
...
def init_fish_user(target_path, conda_prefix, reverse):
    ...
    try:
        rc_content = Path(user_rc_path).read_text()
...
def init_sh_system(target_path, conda_prefix, reverse=False):
    ...
    if exists(conda_sh_system_path):
        conda_sh_system_contents = Path(conda_sh_system_path).read_text()
    else:
...
def modify_easy_install_pth(target_path, conda_source_root):
    ...
    old_contents = Path(target_path).read_text() if isfile(target_path) else ""
